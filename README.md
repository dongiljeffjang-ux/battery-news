...
- **접속 게이트**: 공유 키 `battery2026` (클라이언트 측 간단 게이트, 진짜 인증 아님 → Supabase로 교체 예정).
...
5. **접속 게이트가 진짜 인증이 아님.** battery2026 키는 우회 가능. Supabase 인증으로 교체 예정.
...
1. **Supabase 인증 + DB** ← 토대. 회사 이메일(여러 도메인) 매직링크 로그인. 공유 키 게이트 대체.
   - 진행하던 결정: 매직링크 + 여러 도메인 허용. Supabase 프로젝트 생성 안내까지 갔고 사용자가 일시 중단.
2. **UI 단순화**
3. **좋아요/싫어요** (Supabase 저장) → 피드백 수집
4. **AI 2차 선별** — 규칙 1차 후 AI 재선별. 3번 피드백으로 점수 사전 업데이트.
   - AI는 가장 저렴한 모델로 결정 예정(GPT mini급 vs Claude Haiku급). API 키 아직 없음(발급 필요).
5. **이메일 발송** — 원하면 1회 뉴스 메일 (mailer.py 뼈대 있음)
6. **분석실 AI 인사이트** — office.html 샘플을 진짜 AI로. 권역/application 담당 에이전트가 주 1회 인사이트 도출, 팀장이 웹 검색으로 크리틱.
