#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""권역별 HTML 다이제스트 메일 작성 및 Gmail SMTP 발송."""
...
REGION_ORDER = ["Korea", "China", "Japan", "NorthAmerica", "Europe", "Global"]
...
APP_KO = {
    "EV": "전기차", "ESS": "ESS", "SmallIT": "소형·IT",
    "Micromobility": "e-모빌리티", "Etc": "",
}
MAT_KO = {
    "Cathode": "양극재", "Anode": "음극재", "ElyteSep": "전해질·분리막",
    "CellPack": "셀·팩", "RawMaterial": "원자재", "Etc": "",
}
...
def build_html(fresh, today_str):
    # 중요도 높은 순 정렬 후 권역별 그룹
    fresh = sorted(fresh, key=lambda a: -a.get("score", 3))
    groups = {k: [] for k in REGION_ORDER}
    for a in fresh:
        groups.get(a.get("region", "Global"), groups["Global"]).append(a)
    ...
    for cat in REGION_ORDER:
        ...
        parts.append(
            ...
            f'{REGION_KO[cat]} <span style="color:#999;font-weight:400">'
            f'({len(arts)})</span></h3>')
